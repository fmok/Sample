//
//  FMWebViewController_WK.m
//  Sample
//
//  Created by wjy on 2019/10/9.
...
@interface FMWebViewController_WK ()<
...
@implementation FMWebViewController_WK
...
- (void)viewDidLoad
{
    ...
    [self.webView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(self.view).offset(kNavBarAndStateHeight);
    }];
...
- (void)setupNavigationItem
{
    ...
    [goBackButton setImage:[UIImage imageNamed:@"black-back"] forState:UIControlStateNormal];
    [goBackButton addTarget:self action:@selector(goBackAction:) forControlEvents:UIControlEventTouchUpInside];
    goBackButton.frame = CGRectMake(0, 0, kNavBarHeight, kNavBarHeight);
    self.backItem = [[UIBarButtonItem alloc] initWithCustomView:goBackButton];
...
//    refreshButton.frame = CGRectMake(0, 0, 30, kNavBarAndStateHeight);
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if ([keyPath isEqualToString:NSStringFromSelector(@selector(estimatedProgress))] && object == _webView) {
        TTDPRINT(@"网页加载进度 = %f",_webView.estimatedProgress);
...
{
    if (!_progressView){
        _progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, kNavBarAndStateHeight + 1, self.view.frame.size.width, 2)];
...
//        config.applicationNameForUserAgent = @"Sample";
...
//        [request addValue:[self readCurrentCookieWithDomain:@"https://www.jiemian.com"] forHTTPHeaderField:@"Cookie"];
