...
@interface SVDelegateWrapper : NSObject<UIScrollViewDelegate>
@property(nonatomic, weak) id delegate;
@end

@implementation SVDelegateWrapper
...
@interface SVScrollView : UIScrollView
@property(nonatomic, strong) SVDelegateWrapper *delegateWrapper;
@end
...
- (void)setDelegate:(id<UIScrollViewDelegate>)delegate {
    _delegateWrapper = [SVDelegateWrapper new];
    _delegateWrapper.delegate = delegate;
    [super setDelegate:(id<UIScrollViewDelegate>)_delegateWrapper];
}
...
@property (nonatomic, strong) SVScrollView *scrollView;
...
- (void)setUp
{
    ...
    _scrollView = [[SVScrollView alloc] init];
...
- (void)didEdgePan:(UIPanGestureRecognizer *)edgeGesture {
//    NSLog(@"1231312312312");
}
...
- (void)scrollToNextPage
{
    ...
    offset.x = (int)(offset.x/_itemSize.width) * _itemSize.width;  //纠正偏移
...
// 纠正偏移
- (void)fixCurrentScrollViewPosition
{
    CGPoint offset = self.scrollView.contentOffset;
    offset.x = (int)(offset.x/_itemSize.width) * _itemSize.width;
    [self.scrollView setContentOffset:offset animated:YES];
}
